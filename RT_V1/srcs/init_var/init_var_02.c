...
void	init_objs(t_list **objs)
{
	...
	if (objs)
	{
		...
		(*objs)->content = newSphere(get_pos(0, 400, 0), 100, get_color(255, 0, 0));
		if (!(*objs)->content)
...
*/
		...
		tmp = ft_lstnew(NULL, 0);
		ft_lstadd_end(objs, tmp);
		//tmp->content = newCone_std(get_pos(-800, 0, 500), 1000, 800, get_color(150, 100, 255));
		tmp->content = newCone_std(get_pos(-800, 0, 500), 1000, -1, get_color(150, 100, 255));
