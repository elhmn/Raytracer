...
/*   Updated: 2016/10/17 17:06:02 by bmbarga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			load_light(t_light *l, t_light_tok *t, int id)
{
	if (l && t)
	{
		...
		l->inty = get_inty(get_i(t->id.x, t->id.y, t->id.z),
							get_i(t->is.x, t->is.y, t->is.z));
		l->sp.o = t->pos;
...
static t_list		*add_new_light(t_light_tok *t, int id)
{
	...
	l->content = new_omni_light(get_pos(0, 0, 0),
					get_inty(get_i(0, 0, 0), get_i(0, 0, 0)));
